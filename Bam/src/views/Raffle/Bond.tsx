...
const Bond: React.FC = () => {
  ...
  const handleBuyBonds = useCallback( 
    async (amount: string) => { 
      const tx = await bamFinance.sendBam(amount, raffleAddress);
        addTransaction(tx, {
          summary: `Send ${Number(amount).toFixed(2)} BAM to the raffle ${amount} `,
        });
  ...
  {!!account ? (
    <>
     ...
     <Grid item xs={12} md={12} lg={12} >     
        <h2 style={{ fontSize: '80px', textAlign:'center' }}>Weekly BDAO Raffle</h2>   
        <p style={{ fontSize: '20px', textAlign:'center', color: '#fff' }}>Every week we'll run a raffle for our community where you have the chance to win BDAO tokens just by sending in your freely earned Bam rewards.<br></br> <br></br> 1 Bam =  1 entry and there are unlimited entries per address, the more Bam you send the more chance you have to win. The winner will be chosen at random.</p>                
    ...
    <Grid container justify="center" spacing={3} style={{marginTop: '10px'}}>
        <Grid item xs={12} sm={12} lg={6}>  
            <Card>
              ...
              <p style={{textAlign:'center'}}>Win 1 BDAO this raffle</p>           
              <p style={{textAlign:'center'}}>Bam Price: ${bamPrice}</p>
              <p style={{textAlign:'center'}}>Total Bam Entered: {raffleBals}</p>         
