...
const Stake: React.FC = () => {
  ...
  const [approveStatus, approve] = useApprove(bamFinance.BDAO, bamFinance.contracts.Boardroom.address);

  const tokenBalance = useTokenBalance(bamFinance.BDAO);
        ...
        <CardContent>
          <StyledCardContentInner>
            <StyledCardHeader>
              ...
              <Typography style={{textTransform: 'uppercase', color: '#44a742'}}>  
              <Value value={getDisplayBalance(stakedBalance)} />
              </Typography>
              <Label text={`≈ $${Number(tokenPriceInDollars).toLocaleString('en-US')}`} color="#322f32" />
              <Label text={'BDAO Staked'} color="#322f32" />
            </StyledCardHeader>
            <StyledCardActions>
